...
function init ( ) {
  ...
  sbx.serverInit = function serverInit (env, ctx) {
    ...
    sbx.data = ctx.data.clone();
      ...
      // if (sbx.settings) {
      //     sbx.settings.calibIntercept = -1.1651157547694364;
      //     sbx.settings.calibSlope = 1.3860981055340964;
      // }
  ...
  sbx.clientInit = function clientInit (settings, time, pluginBase, data) {
      reset();
      // if (settings) {
      //     settings.calibIntercept = -1.1651157547694364;
      //     settings.calibSlope = 1.3860981055340964;
      // }
  ...
  sbx.lastCalEntry = function lastCalEntry ( ) {
    return sbx.lastEntry(sbx.data.cals);
  };
  ...
  sbx.scaleEntry = function scaleEntry (entry) {
    ...
    if (entry && entry.scaled === undefined) {
        ...
        if (entry.scaled) {
            // var calibIntercept = -1.1651157547694364;
            // var calibSlope = 1.3860981055340964;

            entry.scaled = sbx.settings.calibSlope * entry.scaled + sbx.settings.calibIntercept;
            entry.scaled = (Math.round(entry.scaled * 100) / 100).toFixed(2);
        }
    }

      return entry && Number(entry.scaled);
      
      // return entry && (Math.round((slope * Number(entry.scaled) + intercept) * 10) / 10).toFixed(1);
  };
  ...
  sbx.roundBGToDisplayFormat = function roundBGToDisplayFormat (bg) {
    return sbx.settings.units === 'mmol' ? Math.round(bg * 100) / 100 : Math.round(bg * 10) / 10;
  };
