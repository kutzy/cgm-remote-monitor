...
client.init = function init(serverSettings, plugins) {
  ...
  function scaleBg(bg) {
    var slope=1.3860981055340964, intercept=-1.1651157547694364;
    if (client.settings.units === 'mmol') {
        return twoDigits(slope * units.mgdlToMMOL(bg) + intercept);
    } else {
        return twoDigits(slope * bg + intercept);
    }
  }
    ...
    if (ago && ago.status !== 'current') {
      ...
      if (currentMgdl < 39) {
        title = s(client.errorcodes.toDisplay(currentMgdl), ' - ') + title;
          ...
          client.sbx.scaleEntry(client.latestSGV);
          var deltaDisplay = client.delta.calc(prevSGV, client.latestSGV, client.sbx).display;
          title = s(scaleBg(currentMgdl)) + s(client.latestSGV.scaled) + s(deltaDisplay) + s(client.direction.info(client.latestSGV).label) + title;
      }
    }
    return title;
  }
    ...
    function adjustCurrentSGVClasses(value, isCurrent) {
      ...
      if (value === 9) {
        ...
        currentBG.text('HIGH');
          ...
          function s(value, sep) { return value ? value + ' ' : sep || ''; }
          currentBG.text(s(scaleBg(value)) + ' ' + s(scaled));
      }
